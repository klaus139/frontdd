...
interface IProps {
  ...
  comment: IComment;
  showReply: IComment[];
  setShowReply: React.Dispatch<React.SetStateAction<IComment[]>>;
  edit?: IComment;
}
...
const CommentList: React.FC<IProps> = ({ 
  ...
  const handleReply = (body: string) => {
    ...
    dispatch(replyComment(data, auth.access_token)as unknown as any)
    setOnReply(false)
  }
  ...
  const handleUpdate = (body: string) => {
    ...
    dispatch(updateComment(newComment, auth.access_token)as unknown as any)
    setEdit(undefined)
  }

  const handleDelete = (comment: IComment) => {
    if(!auth.user || !auth.access_token) return;
    dispatch(deleteComment(comment, auth.access_token)as unknown as any)
  }
